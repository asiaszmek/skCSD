...
class CellModel():
    ...
    def save_electrode_pos(self,directory=''):
        ...
        np.savetxt(electr,fname)
        
    def save_for_R_kernel(self):
        self.save_LFP()
        self.save_electrode_pos()
        
        np.savetxt(os.path.join(new_path, 'somav.txt'), cell.somav)
        coords = np.hstack((self.xmid, self.ymid, self.zmid) 
        )

        np.savetxt(os.path.join(new_path,  'coordsmid_x_y_z'),coords)
        #coordinates of the segment's beginning
        coordsstart = np.hstack( (cell.xstart, cell.ystart, cell.zstart) )
        
        np.savetxt(os.path.join(new_path,  'coordsstart_x_y_z'),coordsstart)
        
        #coordinates of the segment's end
        coordsend = np.hstack((cell.xend, cell.yend, cell.zend))
        
        np.savetxt( os.path.join(new_path, 'coordsend_x_y_z'),coordsend)
        #sdiameter of the segments
        segdiam = np.hstack((cell.diam))
        
        np.savetxt( os.path.join(new_path, 'segdiam_x_y_z'),segdiam)
        
        #length of segments
        np.savetxt( os.path.join(new_path, 'seglength'),cell.length)
        #time in the simulation
        np.savetxt( os.path.join(new_path, 'time'),cell.tvec)
