...
###############################
# TO DO
# sorbarakni az áramokat az összehasonlításhoz -nyaff
...
#morpho.loc<-'/media/BA0ED4600ED416EB/agy/kCSD/progik/bs_futtat/branching_simple2014/network_Hela/morpho_pyr_dori.swc'
#connection.loc<-'/media/BA0ED4600ED416EB/agy/kCSD/progik/bs_futtat/branching_simple2014/network_Hela/suppyrRS_template_dori.hoc'
#connections<-read.table('/media/BA0ED4600ED416EB/agy/kCSD/progik/bs_futtat/branching_simple2014/network_Hela/suppyrRS_template_dori.hoc')
#morpho<-as.matrix(read.table(morpho.loc))
#connections<-read.table(connection.loc)
...
return(list(connection.line=connection.line,t.param=t.param, length.fitted.curve=length.fitted.curve,vonalfun=vonalfun, connection.touched.once=connection.touched.once))
...
#LFP<-as.matrix(read.table("/media/BA0ED4600ED416EB/agy/kCSD/progik/bs_futtat/branching_simple2014/network_Hela/processed_data/LFP_spiketriggerd.txt"))
#elec.kord<-as.matrix(read.table("/media/BA0ED4600ED416EB/agy/kCSD/progik/bs_futtat/branching_simple2014/network_Hela/processed_data/network_elcoords.txt"))



#vonalfun<-branching.result$vonalfun
#morpho<-read.table(morpho.loc)
#connections<-read.table(connection.loc)
...
#Lets put the current in the same order as they were in the simulation
...
if(memb.currents[1,1]!="NA" | seg.length[1,1]!="NA"){
...
current.error.abs<-sum(abs(C.calc-memb.currents.vonal))/sum(abs(memb.currents.vonal))

write.table(memb.currents.vonal,paste(outname,'/membcurr_line',sep=''),col.names=FALSE, row.names=FALSE)

if( mc.error.best > current.error.abs){
M.best.mc<-M
R.best.mc<-R
T.matrix.mc<-t(K.tilda)%*%ginv(K)
mc.error.best<- current.error.abs
C.best.mc<-C.calc
...
#lambda<-R
#memb.currents.smoothed<-array(0,c(M, length(time)))
...
params<-c(params,current.error.abs)
} #current known
...
#all the parameters
write.table(as.matrix(params,ncol=3),paste(outname,'/params',sep=''),col.names=FALSE, row.names=FALSE)
...
write.table(c(M.best.mc, R.best, mc.error.best),paste(outname,'/bestparams_MC__M_R_error',sep=''),col.names=FALSE, row.names=FALSE)
#the transfer matrix in the best situation
