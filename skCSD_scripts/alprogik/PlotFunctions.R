...
ImageAllParCurr<-function(SimPath,locationsKernel,locationsData,SNR, lammbda, R2Plot,M2Plot){
lambda<-lammbda
for(Dlength in 1:length(locationsKernel)){
...
R.all<-R.all[seq(1,length(R.all),,R2Plot)]
M.all<-M.all[seq(1,length(M.all),,M2Plot)]
...
} else { png(paste0(SumPlotPlace,locationsData[Dlength],"skCSD_all_SNRwithBG", SNR, "_lambda",lambda,".png"),width=1000, height=900, pointsize=22)}
plot.new()
par(mfrow=c(length(M.all)+2,length(R.all)),oma=c(4,2,4,4))#,mar=c(1,1,1,1))
#par(mfrow=c(length(M.all)+2,length(R.all)),oma=c(2,3,4,2))#,mar=c(1,1,1,1))
...
title(main="ksCSD Reconstruction", outer=TRUE, cex=3)
...
for(m in 1:length(M.all)){
  for(r in 1:length(R.all)){
    ...
    R<-R.all[r]
    #Reading in the LFP and adding noise
currName<-paste0(outname,outname1,"/skCSDall_M",M,"_R",R,"_SNR", SNR, "_lambda",lambda)
    if(file.exists(currName)) {
      skCSD.all<-as.matrix(read.table(currName)) 
                               }else next
...
if(r==1) mtext(paste0("M",M),side=2)
if(r==length(R.all)) {axis(4,at=round(seq(1,SegNb-10,,4)),las=2)}
if(m==length(M.all)) axis(1,round(seq(0,max(times2Plot)-10,,5)),las=2)


#image(t(skCSD.all),main=paste("skCSD M:",M, "R",R),col=col2,zlim=c(-ExtVal,ExtVal),xaxt="n",yaxt="n")
...
ElNum_ErrorPlot<-function(SimPath,locationsKernel,locationsData,SNR, lammbda){
lambda<-lammbda
EL2ErrorAll<-numeric()
EL2ErrorFixAll<-numeric()
Besterror<-array(NA,c(3,length(locationsData)))
BesterrorL2<-array(NA,c(3,length(locationsData)))
BesterrorCV<-array(NA,c(3,length(locationsData)))
BesterrorFix<-numeric()
...
R.all<-unique(params[1,])#[3]
M.all<-unique(params[2,])#[5]
withoutSmoothing<-c(1:length(R.all))
withfixRsmooth<-c((length(R.all)+1):(2*length(R.all)))
...
EL2Error<-read.table( paste(outname,outname1,"/EL2ErrorSoma","_SNR",
 SNR, "_lambda",lambda,sep=""))[withoutSmoothing,]


BesterrorL2[,Dlength]<-c( R.all[which(EL2Error==min(EL2Error),arr.ind=TRUE)[1]], 
                        M.all[which(EL2Error==min(EL2Error),arr.ind=TRUE)[2]],min(EL2Error))

write.table(BesterrorL2, paste(outname,outname1,"/L2Error_WSoma","_SNR",
                             SNR, "_lambda",lambda,sep=""))
png(paste0(outname,"/",locationsData[Dlength],"L2_ErrorvsR_ElNumb",ElNumb,"_SNR", SNR, "_lambda",lambda),width=600, height=600,pointsize=20)
par(mar=c(4,4,4,6))
yLim<-min(1.1, max(EL2Error))
matplot(R.all, EL2Error,main="Error dependence on basis width",t="l",
        xlab="Basis Width",ylab="L2 error",lwd=2,ylim=c(min(EL2Error),yLim))
par(xpd=TRUE)
legend("topleft",c(paste(M.all)),pch=20,col=1:6,title="M")
par(xpd=FALSE)
dev.off()
...
EL2ErrorFix<-read.table( paste(outname,outname1,"/EL2ErrorSoma","_SNR", 
SNR, "_lambda",lambda,sep=""))[withfixRsmooth,]




E2SegNormSoma<-read.table( paste(outname,outname1,"/E2SegNormrSoma","_SNR", SNR, "_lambda",lambda,sep=""))[withoutSmoothing,]
CrossVal<-read.table( paste(outname,outname1,"/ECrossVal","_SNR", SNR, "_lambda",lambda,sep=""))[withoutSmoothing,]

ChosenError<-EL2ErrorFix #E2SegNormSoma

png(paste0(outname,"/ErrorvsR_ElNumb",ElNumb,"_SNR", SNR, "_lambda",lambda))
 par(mar=c(4,4,4,6))
matplot(R.all, EL2ErrorFix,main="Error dependence on basis width",t="l",
 xlab="Basis Width",ylab="fix smoothed error",ylim=c(min(EL2ErrorFix),2))
 par(xpd=TRUE)
  legend(max(R.all)+10,mean(2,min(EL2ErrorFix)),c(paste(M.all)),pch=20,col=1:6,title="M")
  par(xpd=FALSE)
dev.off()

png(paste0(outname,"/ErrorSegvsR_ElNumb",ElNumb,"_SNR", SNR, "_lambda",lambda))
 par(mar=c(4,4,4,6))
matplot(R.all, ChosenError,main="Error dependence on basis width",t="l",
 xlab="Basis Width",ylab="fix smoothed error",ylim=c(min(ChosenError),2))
 par(xpd=TRUE)
  legend(max(R.all)+10,mean(2,min(ChosenError)),c(paste(M.all)),pch=20,col=1:6,title="M")
  par(xpd=FALSE)
dev.off()


#Besterror[,Dlength]<-c( R.all[which(ChosenError==min(ChosenError),arr.ind=TRUE)[1]], 
#M.all[which(ChosenError==min(ChosenError),arr.ind=TRUE)[2]],min(ChosenError))

  Besterror[,Dlength]<-c( R.all[which(ChosenError==min(ChosenError),arr.ind=TRUE)[1]], 
  M.all[which(ChosenError==min(ChosenError),arr.ind=TRUE)[2]],min(ChosenError))
                        
write.table(Besterror, paste(outname,outname1,"/BestEL2ErrorFix_WSoma","_SNR",
 SNR, "_lambda",lambda,sep=""))


ChosenError<-CrossVal
png(paste0(outname,"/ErrorCVvsR_ElNumb",ElNumb,"_SNR", SNR, "_lambda",lambda))
 par(mar=c(4,4,4,6))
matplot(R.all, ChosenError,main="Error dependence on basis width",t="l",
 xlab="Basis Width",ylab="fix smoothed error",ylim=c(min(ChosenError),0.5))
 par(xpd=TRUE)
  legend(max(R.all)+10,mean(0.5,min(ChosenError)),c(paste(M.all)),pch=20,col=1:6,title="M")
  par(xpd=FALSE)
dev.off()
BesterrorCV[,Dlength]<-c( R.all[which(ChosenError==min(ChosenError),arr.ind=TRUE)[1]], 
M.all[which(ChosenError==min(ChosenError),arr.ind=TRUE)[2]],min(ChosenError))
write.table(BesterrorCV[,Dlength],paste(outname,outname1,"/CVBest_WSoma","_SNR",
 SNR, "_lambda",lambda,sep=""))

write.table(Besterror, paste(outname,outname1,"/CVError_WSoma","_SNR",
 SNR, "_lambda",lambda,sep=""))

#Columns- different Ms, raws, different Rs
#EL2ErrorAll<-cbind(EL2ErrorAll,EL2Error)
#EL2ErrorFixAll<-cbind(EL2ErrorFixAll, EL2ErrorFix)
...
BestEMatr<-matrix(c(Besterror),nrow=3)

return(list(BestEMatr=BesterrorL2,BestEMatrFix=BestEMatr,BesterrorCV=BesterrorCV))
}
