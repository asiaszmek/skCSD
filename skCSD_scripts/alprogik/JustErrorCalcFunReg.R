...
JustErrorCalcFunReg <-
  function(SimPath,locationsKernel,locationsData,SNR,scripts_dir) {
      Reg <- TRUE
    M <- 512
    ...
    for (Dlength in 1:length(locationsKernel)) {
      inname <- paste(SimPath, locationsKernel[Dlength],sep="/")
      outname <- paste(SimPath, locationsData[Dlength], sep="/")
      ...
      sigma <- as.matrix(read.table(paste(outname, "elprop", sep = "")))[2]
      ...
      morpho <- as.matrix(read.table(paste0(outname, 'segcoordinates.txt')))
      ...
      for (lamb in 1:length(lambda.all)) {
        for (r in 1:length(R.all)) {
          ...
          R <- R.all[r]
          el2ignorename <-
            paste(inname,"/El2Ignore_M", M, "_R", R, sep = "")
          ...
          if (file.exists(
            ...
            skCSD.all <- array(0, c(seg.nb, dim(LFP)[2]))
            skCSD.all.part <-
              (4 * pi * 0.5) ^ 2 * as.matrix(read.table(
                paste(
                  inname,
                  "/ksCSD_Matr",
                  M,
          ...
          SameplaceAll <- readLines(paste(inname,"sameplace.txt",sep="/"))
          seg.nb <-
            max(as.matrix(read.table(
              paste(inname,  'connection_touched_once', sep = '/')
            )))
          ...
          for (i in 1:seg.nb) {
            ...
            if (length(sameplace) == 1)
              skCSD.all[i, ] <- skCSD.all.part[sameplace, ]
            if (length(sameplace) > 1)
              skCSD.all[i, ] <-
              as.matrix(colSums(skCSD.all.part[c(sameplace), ]), nrow = 1)
          }
          ...
          source(paste(scripts_dir,"KernSmoothDistance.R",sep="/"))
          source(paste(scripts_dir,"errorCalcFun.R",sep="/"))
          
          for (bw in 1:length(BandwidtAll)) {
            memb.currents.smoothed <-
              KernSmoothDistance(BandwidtAll[bw],
                                 inname,
                                 outname,
                                 BandwidtAllName[bw])
            EL2ErrorVarR[r + (bw - 1) * length(R.all), lamb] <-
              L2Error(skCSD.all, memb.currents.smoothed)[[1]]
            EL1ErrorVarR[r + (bw - 1) * length(R.all), lamb] <-
              L1Error(skCSD.all, memb.currents.smoothed)[[1]]
          ...
          EL2ErrorVarR[r + length(BandwidtAll) * length(R.all), lamb] <-
            L2Error(skCSD.all, memb.currents.line)[[1]]
          EL1ErrorVarR[r + length(BandwidtAll) * length(R.all), lamb] <-
            L1Error(skCSD.all, memb.currents.line)[[1]]
      ...
      write.table(
        El2ErrorRaw,
        paste(inname,  "/ErrorL2Raw_Noise", "_SNR", SNR, sep = ""),
        row.names = FALSE,
        col.names = FALSE
      )
      write.table(
        El1ErrorRaw,
        paste(inname,  "/ErrorL1Raw_Noise", "_SNR", SNR, sep = ""),
        row.names = FALSE,
        col.names = FALSE
      )
      ...
      write.table(
        EL2ErrorVarR,
        paste(inname, "/EL2ErrorVarR", "_SNR", SNR, sep = ""),
        row.names = FALSE,
        col.names = FALSE
      )
      write.table(
        EL1ErrorVarR,
        paste(inname,  "/EL1ErrorVarR", "_SNR", SNR, sep = ""),
        row.names = FALSE,
        col.names = FALSE
      )
...
#     
#     if (file.exists(paste0(outname, outname1)) == FALSE)
#       dir.create(paste0(outname, outname1))
#     
#     SameplaceAll <-
#       readLines(paste0(inname, outname1, "/sameplace.txt"))
#     seg.nb <-
#       max(as.matrix(read.table(
#         paste(inname, outname1, '/connection_touched_once', sep = '')
#       )))
#     LFP <- as.matrix(read.table(paste(outname, "/myLFP", sep = "")))
#     
#     if (SNR != 0) {
...
#     
#     ElCoords <-
#       as.matrix(read.table(paste(inname, "elcoord_x_y_z", sep = "")))
#     ElCoords <- matrix(ElCoords, ncol = 3)
...
#         
#         el2ignorename <-
#           paste(inname, outname1, "/El2Ignore_M", M, "_R", R, sep = "")
#         if (file.exists(el2ignorename)) {
#           El2Ignore <- c(as.matrix(read.table(el2ignorename)))
#           LFP <- LFP[-El2Ignore, ]
#         }
...
#     write.table(
#       ErrorCV,
#       paste(outname, outname1, "/ErrorCV_NoiseElec", "_SNR", SNR, sep = ""),
#       row.names = FALSE,
#       col.names = FALSE
#     )
#     
...
#     write.table(alma1[[3]][[iter1]],
#                 filename1,
#                 row.names = FALSE,
#                 col.names = FALSE)
#     write.table(alma1[[2]][[iter1]],
#                 filename2,
#                 row.names = FALSE,
#                 col.names = FALSE)
#   }
...
#   png(paste0(SumPlotPlace, "Y_L1vsElnumb.png"),
#       width = 500,
#       height = 500)
#   #par(xpd=FALSE)
#   par(cex = 1.5)
...
#     pch = 20,
#     lwd = 2,
#     xlab = "Number of Electrodes",
#     ylab = "L1 Error"
#   )
#   points(unique(ElNumb),
#          colMeans(MinErrorArray),
#          col = "GRAY",
#          pch = 20)
#   legend(
#     "topright",
#     c("Grid", "Rand 1", "Rand 2", "Rand 3", "Rand 4", "Rand 5", "Mean"),
#     pch = 20,
#     col = c(1:6, "GRAY"),
#     title = "Setup"
#   )
...
#   
#   #Yrot:
#   png(paste0(SumPlotPlace, "BS_L1vsElnumb.png"),
#       width = 500,
#       height = 500)
#   #par(xpd=FALSE)
#   par(cex = 1.5)
...
#         SimPath, locationsKernel[Which2Plot], "/SmoothingWidth"
#       )))
#     
#     Error2Plot <-
#       as.matrix(read.table(paste0(
#         SimPath, locationsKernel[Which2Plot], "/ErrorTable"
#       )))
#     matplot(
#       Error2Plot[1:length(R.all), ],
#       t = "l",
#       ylim = range(Error2Plot),
#       xlim = c(1, length(R.all) * (length(SmoothWidth) + 1)),
#       xaxt = "n",
#       xlab = "Basis Width",
#       ylab = "Relative Error",
#       lwd = 1.5
...
#       matlines(length(R.all) * (sw - 1) + 1:length(R.all) ,
#                Error2Plot[((sw - 1) * length(R.all) + 1):(sw * length(R.all)), ],
#                lwd = 1.5)
#     }
...
#     mtext(
#       c(round(SmoothWidth, 3), " - "),
#       side = 3,
#       at = c(1:(length(SmoothWidth) + 1) * length(R.all) - length(R.all) / 2),
#       line = -1
#     )
...
#     Error2Plot <-
#       as.matrix(read.table(paste0(
#         SimPath, locationsKernel[iter1], "/ErrorTable"
#       )))
#     #32-es símításos
#     CurrMin <- min(Error2Plot[(2 * length(R.all) + 1):(3 * length(R.all)), ])
#     IndecesMin <-
#       which(Error2Plot[(2 * length(R.all) + 1):(3 * length(R.all)), ] == CurrMin, arr.ind =
#               TRUE)
...
#   SmoothWidth <-
#     as.matrix(read.table(paste0(
#       SimPath, locationsKernel[iter1], "/SmoothingWidth"
#     )))
#   matplot(
#     unique(ElNumb),
#     t(MinError),
...
#   
#   
#   png(paste0(SumPlotPlace, "/ElnumbvsParams_", CellType, ".png"),
#       width = 800)
...
#     xlab = "R",
#     xaxt = "n",
#     main = "Estimation Error of CSD on Gang"
#   )
...
#   axis(
#     3,
#     at = (1:4) * length(R.all) - 0.5 * length(R.all),
#     labels = c("8x8", "6x5", "12x10a", "12x10b"),
#     tick = FALSE,
#     line = -1
#   )
#   
#   abline(v = c((1:(
#     length(locationsKernel) - 1
#   )) * length(R.all) + 0.5), col = 1)
#   legend(
#     "topleft",
#     c(paste(lambda.all)),
#     pch = 20,
...
#   mtext(
#     round(alma[[2]][[Which2Plot]], 3),
#     side = 3,
#     at = c(1:length(alma[[2]][[Which2Plot]]) * length(lambda.all) - length(lambda.all) /
#              2),
#     line = -1
#   )
...
#           
#           CurrNew <-
#             KernSmoothDistance(30, inname, outname, outname1, "30")
#           
#           el2ignorename <-
#             paste(inname, outname1, "/El2Ignore_M", M, "_R", R, sep = "")
#           if (file.exists(el2ignorename)) {
...
#             sameplace <- numeric()
#             sameplace <-
#               as.integer(unlist(strsplit(SameplaceAll, " ")[i]))
#             #Plot the current related ti the different segments
#             #  matplot(cbind(t(C.calc[c(sameplace),])),t='l', xlab="Time steps", ylab="ksCSD", main="Values of current at the same location")
#             if (length(sameplace) == 1)
#               skCSD.all[i, ] <- skCSD.all.part[sameplace, ]
...
#           }
#           El1ErrorRaw[r, lamb] <-
#             L1Error(skCSD.all[, Timeframe], CurrNew[, Timeframe])[[1]]
#           
#           
#           #      cvErrorOut<-cross.valid(LFP,Tmatr,1:dim(Tmatr)[1])
#           #ErrorCV[r,lamb]<-cvErrorOut[[1]]
...
# 
# korte <-
#   JustL1RegTime(SimPath, locationsKernel, locationsData, SNR, c(1, 1000))
# 
# 
# 
...
#       El1ErrorRawAllGang <-
#         L1Error(skCSD.all[, TimeframeEval], CurrNew[, TimeframeEval])[[1]]
#       
#       El1ErrorRawAllGangAll <-
#         c(El1ErrorRawAllGangAll, El1ErrorRawAllGang)
#       #      cvErrorOut<-cross.valid(LFP,Tmatr,1:dim(Tmatr)[1])
#       #ErrorCV[r,lamb]<-cvErrorOut[[1]]
#       cat("Numb to rteplace:", length(R.all) * (lamb - 1) + r)
#       #CVConfig[[length(R.all)*(lamb-1)+ r]]<-as.matrix(cvErrorOut[[4]])
...
#       write.table(
#         c(r, lamb),
#         paste(outname, outname1, "/L1Compare_BestParams", sep = ""),
#         row.names = FALSE,
#         col.names = FALSE
#       )
#       
...
# 
# 
# JustL1RegTimeWBestParams(SimPath,
#                          locationsKernel,
#                          locationsData,
#                          SNR,
#                          c(1, 1000),
...
#       
#       ElCoords <-
#         as.matrix(read.table(paste(outname, "elcoord_x_y_z", sep = "")))
#       ElCoords <- matrix(ElCoords, ncol = 3)
#       Xel <- unique(ElCoords[, 2])
#       XelNb <- length(Xel)
#       Yel <- unique(ElCoords[, 3])
...
#           
#           CurrNew <-
#             KernSmoothDistanceSpTrig(30, inname, outname, outname1, "30")
#           
#           el2ignorename <-
#             paste(inname, outname1, "/El2Ignore_M", M, "_R", R, sep = "")
#           if (file.exists(el2ignorename)) {
