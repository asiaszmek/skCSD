...
#membcurr<-as.matrix(read.table(paste0(DataFolder,'membSpTrig')))
 ...
 membcurr<-KernSmoothDistanceSpTrig(30,DataFolder, DataFolder, outname1,"30")
...
dir.create(paste0(DataFolder, "/Plots"))
for(t2 in 1:dim(LFP)[2]){
TimeInstant<-t2
png(paste0(DataFolder,"/Plots/SpTrig_",t2,".png"))
par(mfrow=c(2,2))
...
segments(segstart[c(dimseg:1),xAEl], segstart[c(dimseg:1),yAEl], segend[c(dimseg:1),xAEl], segend[c(dimseg:1),yAEl],lwd=1)
...
#symbols(segmid[c(dimseg:1)[ToPlot],xA],segmid[c(dimseg:1)[ToPlot],yA], circles=c(diff(range(segstart[,xA]))/Plotwidth* abs(membcurr[,TimeInstant])/ExtVal), add=TRUE, bg=colors()[182], fg=colors()[182], inches=FALSE)
...
plot(1,type='n',xlim=Xrange,ylim=Yrange,xlab='x (um)',ylab='y (um)',main="skCSD",asp=1)
rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = "gray")
...
#symbols(segmid[c(dimseg:1)[ToPlot],xA],segmid[c(dimseg:1)[ToPlot],yA], circles=c(diff(range(segstart[,xA]))/Plotwidth* abs(skCSD[,TimeInstant])/ExtVal), add=TRUE, bg=colors()[182], fg=colors()[182], inches=FALSE)
